...
```{r}
head( spatialwidget::capitals )
```

And convert it into pseudo-geojson ready to be parsed by javascript inside an htmlwidget

```{r}
# js <- spatialwidget::widget_point( data = spatialwidget::capitals, fill_colour = "country", legend = TRUE)
# 
...
Notice the `fill_colour` columns is now a hex colour, and the `geometry` have been converted into `Point` coordinates. 
...
The `data` is returned as **pseudo-GeoJSON** because some plotting libraries can use more than one geometry, such as [`mapdeck::add_arc()`](https://github.com/SymbolixAU/mapdeck/blob/master/R/map_layer_arc.R), which uses an origin and destination. So spatialwidget needs to handle multiple geometries.
...
```{r}
l <- spatialwidget::widget_point( capitals[1:2, ], fill_colour = "country", legend = T )
...
```{r}
l <- spatialwidget::widget_line( roads[1:2, ], stroke_colour = "ROAD_NAME", legend = T )
...
```{r}
l <- spatialwidget::widget_polygon( melbourne[1:2, ], fill_colour = "AREASQKM16", legend = F)
...
The `spatialwidget::api::` namespace has 5 functions for converting your data into pseudo-geojson. Here are their definitions, the input data they expect and the type of output. 
...
When converting an `sf` object to GeoJSON it will typically create a FeatureCollection. 'Atomising' means it treats each row of the `sf` as it's own Feature, without combining them into a Feature Collection.
...
If we set it to 'atomise' then an array of Features is returned
...
**downcasting** is the process of converting MULTI objects to their simpler form. For example, an MULTIPOINT will be down-cast to POINTs (one row/value for each of the multipoints).
...
Here I describe how each of the functions work.
...
```{r}
df <- as.data.frame( capitals )
coords <- sf::st_coordinates( capitals )
df[, c("lon","lat")] <- coords
df$geometry <- NULL
