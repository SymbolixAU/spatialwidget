...
inline Rcpp::StringVector to_geojson_mesh(
  ...
  //Rcpp::Rcout << "ib nrow: " << ib.nrow() << std::endl;
  //Rcpp::Rcout << "ib ncol: " << ib.ncol() << std::endl;
  //Rcpp::Rcout << "vb nrow: " << vb.nrow() << std::endl;
  //Rcpp::Rcout << "vb ncol: " << vb.ncol() << std::endl;

  int n_cols = ib.ncol(); // number of geometries
  int i, j;
  ...
  for( i = 0; i < n_cols; i++ ) {
    ...
    // iterate over each column if 'ib'; this tells the columns of 'vb' to use to make the polygon
    ...
    Rcpp::NumericVector these_cols = ib( _, i );
    //Rcpp::Rcout << "these_cols: " << these_cols << std::endl;
    writer.String("type");
